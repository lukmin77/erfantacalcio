'use client'
import { Alert, Box, Button, Card, CardContent, CardHeader, CircularProgress, Grid, MenuItem, Select, type SelectChangeEvent, Stack, TextField, Tooltip, Typography, FormControl, InputLabel, Paper } from "@mui/material";
...
export default function Giocatori() {
    ...
    const [selectedTrasferimentoId, setSelectedTrasferimentoId] = useState<number>();
    const [selectedTrasferimentoStagione, setSelectedTrasferimentoStagione] = useState<string>();
    const trasferimentiList = api.trasferimenti.list.useQuery({ idGiocatore: selectedGiocatoreId! }, { enabled: !!selectedGiocatoreId });
    ...
    const giocatoreOne = api.giocatori.get.useQuery({ idGiocatore: selectedGiocatoreId! }, { enabled: !!selectedGiocatoreId, refetchOnWindowFocus: false, refetchOnReconnect: false });
    const squadreList = api.squadre.list.useQuery(undefined, { refetchOnWindowFocus: false, refetchOnReconnect: false });
    const squadreSerieAList = api.squadreSerieA.list.useQuery(undefined, { refetchOnWindowFocus: false, refetchOnReconnect: false });
    ...
    const trasferimentoOne = api.trasferimenti.get.useQuery({ idTrasferimento: selectedTrasferimentoId! }, { enabled: !!selectedTrasferimentoId, refetchOnWindowFocus: false, refetchOnReconnect: false });
    ...
    const actionEdit = (idTrasferimento: string, stagione: string) => {
        return (
            <div>
                <Tooltip title={"Modifica"} onClick={() => handleEditTrasferimento(+idTrasferimento, stagione)} placement="left">
                    <EditNoteIcon />
                </Tooltip>
            </div>
        )
    ...
    useEffect(() => {
        if (trasferimentiList.isError) {
            setErrorMessageTrasferimenti('Si è verificato un errore in fase di caricamento dei trasferimenti');
        }
    ...
    useEffect(() => {
        if (!giocatoreOne.isFetching && giocatoreOne.isSuccess && giocatoreOne.data) {
    ...
    useEffect(() => {
        if (!trasferimentoOne.isFetching && trasferimentoOne.isSuccess && trasferimentoOne.data) {
            setTrasferimento(trasferimentoOne.data);
            setErrorMessageTrasferimento('');
            setMessageTrasferimento('');
        }
    }, [trasferimentoOne.data, trasferimentoOne.isSuccess, trasferimentoOne.isFetching]);

    //#region anagrafica

    const handleGiocatoreSelected = async (idGiocatore: number | undefined, nome: string | undefined) => {
    ...
    const handleUpsertGiocatore = async (event: React.FormEvent<HTMLFormElement>) => {
        event.preventDefault();
        setErrorMessageGiocatore('');
        setMessageGiocatore('');
        const responseVal = GiocatoreSchema.safeParse(giocatore);
        if (!responseVal.success) {
            setErrorMessageGiocatore(responseVal.error.issues.map(issue => `campo ${issue.path.toLocaleString()}: ${issue.message}`).join(', '));
        }
        else {
            try {
                ...
                setErrorMessageGiocatore('Si è verificato un errore nel salvataggio dell\'anagrafica giocatore');
            }
        }
    };
    ...
    const handleDeleteGiocatore = async () => {
        ...
        if (selectedGiocatoreId) {
            try {
                ...
                setErrorMessageGiocatore('Si è verificato un errore nell\'eliminazione del giocatore');
            }
        }
    };
    ...
    const handleEditTrasferimento = async (_idTrasferimento: number, stagione: string) => {
    ...
    const handleUpsertTrasferimento = async (event: React.FormEvent<HTMLFormElement>) => {
        event.preventDefault();
        ...
        if (!responseVal.success) {
            setErrorMessageTrasferimento(responseVal.error.issues.map(issue => `campo ${issue.path.toLocaleString()}: ${issue.message}`).join(', '));
        }
        else {
            try {
                ...
                setErrorMessageTrasferimento('Si è verificato un errore nel salvataggio del trasferimento giocatore');
            }
        }
    };
    ...
    const handleDeleteTrasferimento = async () => {
        ...
        try {
            ...
            setErrorMessageTrasferimento('Si è verificato un errore nell\'eliminazione del trasferimento');
        }
    ...
    const handleSelectChange = (event: SelectChangeEvent, form: 'anagrafica' | 'trasferimento') => {
        if (form === 'anagrafica')
            setGiocatore(prevState => ({ ...prevState, [event.target.name]: event.target.value }));
        if (form === 'trasferimento')
            setTrasferimento(prevState => ({ ...prevState, [event.target.name]: event.target.value === '0' ? null : parseInt(event.target.value) }));
    };
    ...
    return (
        <Stack direction="column" spacing={1} justifyContent="space-between" paddingTop={2} paddingBottom={2}>
            ...
            <Card sx={{ p: 0 }}>
                ...
                <CardContent>
                    <Box component="form" onSubmit={handleUpsertGiocatore} noValidate sx={{ mt: 1 }}>
                            ...
                            <Grid item xs={8}>
                                <Stack direction="row" spacing={1} justifyContent="flex-start" >
                                    <TextField
                                        ...
                                        onChange={(event: React.ChangeEvent<HTMLInputElement>) => handleInputChange(event, 'anagrafica')}
                                    />
                                    <FormControl sx={{ m: 1, minWidth: 120 }} size="small">
                                        ...
                                        <Select size='small'
                                            ...
                                            onChange={(event: SelectChangeEvent) => handleSelectChange(event, 'anagrafica')}
                                        >
                                            {ruoliList.map((item) => (
                                ...
                                <Stack direction="row" spacing={1} justifyContent="space-between" >
                                    {messageGiocatore && (
                                        <Stack sx={{ width: '100%' }} spacing={0}>
                                            <Alert icon={<CheckIcon fontSize="inherit" />} severity="success">{messageGiocatore}</Alert>
                                        </Stack>
                                    )}
                                    {errorMessageGiocatore && (
                                        <Stack sx={{ width: '100%' }} spacing={0}>
                                            <Alert icon={<CheckIcon fontSize="inherit" />} severity="error">{errorMessageGiocatore}</Alert>
                                        </Stack>
                                    )}
                                </Stack>
                            </Grid>
            ...
            {selectedGiocatoreId !== undefined && (
                <Paper elevation={3}>
                    <Card sx={{ p: 0 }}>
                        <CardHeader title="Trasferimento giocatore" subheader="Inserisci/aggiorna trasferimento" titleTypographyProps={{ variant: 'h4' }} />
                        <CardContent>
                            <Box component="form" onSubmit={handleUpsertTrasferimento} noValidate sx={{ mt: 1 }}>
                                <Grid container spacing={0}>
                                    <Grid item xs={8}>
                                        <Stack direction="row" spacing={1} justifyContent="flex-start" >
                                            <TextField
                                            ...
                                            />
                                            <FormControl sx={{ m: 1, minWidth: 120 }} size="small">
                                                <InputLabel id="select-label-fantasquadra">Fantasquadra</InputLabel>
                                                <Select size='small'
                                                    variant='outlined'
                                                    ...
                                                    onChange={(event: SelectChangeEvent) => handleSelectChange(event, 'trasferimento')}
                                                >
                                                    {squadre.map((item) => (
                                                        <MenuItem key={item.id} value={item.id?.toLocaleString()} sx={{ color: 'black' }}>
                                                            {item.label}
                                                        </MenuItem>
                                                    ))}
                                                </Select>
                                            ...
                                            <TextField
                                                ...
                                                //label='Data Cessione'
                                                name="dataCessione"
                                                value={trasferimento.dataCessione !== null ? convertFromIsoToDatetimeMUI(dayjs(trasferimento.dataCessione).toISOString()) : null}
                                                onChange={(event: React.ChangeEvent<HTMLInputElement>) => handleInputChange(event, 'trasferimento')}
                                            />
                                        </Stack>
                                    </Grid>
                                        ...
                                        <Stack direction="row" spacing={1} justifyContent="flex-end" >
                                            ...
                                            {selectedTrasferimentoStagione === Configurazione.stagione && (
                                                <Button
                                                    ...
                                                    sx={{ mt: 3, mb: 2 }}
                                                >
                                                    {selectedTrasferimentoId ? 'Aggiorna trasferimento' : 'Aggiungi trasferimento'}
                                                </Button>
                                            )}
                                        </Stack>
                                    </Grid>
                                    <Grid item xs={12}>
                                        <Stack direction="row" spacing={1} justifyContent="space-between" >
                                            {messageTrasferimento && (
                                                <Stack sx={{ width: '100%' }} spacing={0}>
                                                    <Alert icon={<CheckIcon fontSize="inherit" />} severity="success">{messageTrasferimento}</Alert>
                                                </Stack>
                                            )}
                                            {errorMessageTrasferimento && (
                                                <Stack sx={{ width: '100%' }} spacing={0}>
                                                    <Alert icon={<CheckIcon fontSize="inherit" />} severity="error">{errorMessageTrasferimento}</Alert>
                                                </Stack>
                                            )}
                                        </Stack>
                                    </Grid>
            ...
            {trasferimentiList.isLoading && !trasferimentiList.isSuccess && selectedGiocatoreId !== undefined ? (
