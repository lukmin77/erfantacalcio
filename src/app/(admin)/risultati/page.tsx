...
import { Grid, Card, CardContent, Box, Stack, CardHeader, Select, MenuItem, type SelectChangeEvent, LinearProgress, CircularProgress, FormControl, InputLabel, Paper } from "@mui/material";
...
export default function Risultati() {
    ...
    const partiteList = api.risultati.getGiornataPartite.useQuery({ idCalendario: selectedIdCalendario!, includeTabellini: true, backOfficeMode: true }, { enabled: !!selectedIdCalendario });
                ...
                {calendarioList.isLoading ? (
                    ...
                    <Paper elevation={3}>
                        <Card sx={{ maxWidth: 600, p: 1 }}>
                            <CardHeader title="Aggiornamento risultati" subheader="aggiornamento risultati partite, seleziona la giornata da caricare" titleTypographyProps={{ variant: 'h4' }} />
                            <CardContent>
                                <Box sx={{ p: 1 }}>
                                ...
                                {selectedIdCalendario !== selectedGiornata?.idCalendario ? (
                                    ...
                                    <>
                                        {selectedGiornata && (
                                            <CardPartiteAdmin giornata={selectedGiornata}></CardPartiteAdmin>
                                        )}
                                    </>
                                )}
                            </CardContent>
