...
import { Box, Button, ButtonGroup, CircularProgress, Divider, Grid, Tooltip, Zoom, Typography, useMediaQuery, useTheme, IconButton, Card, CardHeader, CardMedia, Slide } from '@mui/material';
import { AccessAlarm, Ballot, CalendarMonth, EmojiEvents, Looks3Outlined, Looks4Outlined, Looks5Outlined, LooksOneOutlined, LooksTwoOutlined, Login, PendingActions, Home } from '@mui/icons-material';
...
export default function HomePage() {
  ...
  const torneiList = api.tornei.list.useQuery(undefined, { enabled: frame === FrameType.defaultHome, refetchOnWindowFocus: false, refetchOnReconnect: false });
  ...
  const [isCalendarioRecuperi, setIsCalendarioRecuperi] = useState<boolean>(false);
  ...
  const calendarioList = girone && !isCalendarioAttuale && !isCalendarioRecuperi
    ? api.calendario.listByGirone.useQuery(girone, { enabled: true, refetchOnWindowFocus: false, refetchOnReconnect: false })
    : isCalendarioAttuale 
      ? api.calendario.listAttuale.useQuery(undefined, { enabled: isCalendarioAttuale, refetchOnWindowFocus: false, refetchOnReconnect: false })
      : api.calendario.listRecuperi.useQuery(undefined, { enabled: isCalendarioRecuperi, refetchOnWindowFocus: false, refetchOnReconnect: false });
  ...
  const handleClassifica = useCallback((nomeTorneo: string, fase: string | null) => {
    if (torneiList.data) {
      setTorneo(torneiList.data.find(c => c.nome === nomeTorneo && c.gruppoFase === fase));
    }
  }, [torneiList.data]);
  ...
  }, [torneiList.data, torneiList.isSuccess, torneiList.isFetching, handleClassifica]);

  const handleCalendario = (girone: number | undefined, isAttuale: boolean, onlyRecuperi: boolean) => {
    setTitleModalCalendario(girone ? `Calendario girone ${girone}` : isAttuale ? `Calendario ultime partite` : `Calendario partite da recuperare`);
    setGirone(girone);
  ...
  const handleChangeRosa = (newFrame: FrameType, newIdSquadra?: number, newSquadra?: string) => {
    setIdSquadra(newIdSquadra);
    setSquadra(newSquadra);
    handleChangeFrame(newFrame);
  };

  const handleChangeStatistica = (newFrame: FrameType, newIdSquadra?: number) => {
    setIdSquadra(newIdSquadra);
    handleChangeFrame(newFrame);
  };
    ...
    <>
      <Grid container spacing={0}>
        {frame === FrameType.defaultHome && isXs && session?.user && (
          <>
            <Grid item xs={6}>
              <Typography variant='h5'>Bentornato {session.user.presidente}</Typography>
            </Grid>
            <Grid item xs={6} display={'flex'} justifyContent={'flex-end'}>
              <Tooltip title="Schiera formazione" placement="top-start">
                <IconButton onClick={() => handleChangeFrame(FrameType.schieraFormazione)}>
        ...
        {frame === FrameType.defaultHome && !torneiList.isLoading && (
          <>
            <Zoom in={frame === FrameType.defaultHome} >
              <Grid item xs={12} sm={6} sx={!isXs ? { pl: '2px', pr: '15px', pt: '15px' } : {}}>
                <Classifica nomeTorneo={torneo?.nome ?? ''} idTorneo={torneo?.idTorneo} gruppo={torneo?.gruppoFase ?? ''}></Classifica>
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', '& > *': { m: 1, }, }}>
                  <ButtonGroup size="small" color='primary' aria-label="Small button group">
                    <Button onClick={() => handleClassifica('Campionato', null)} startIcon={<CalendarMonth />} sx={isXs ? { fontSize: '10px' } : {}}>Campionato</Button>
                    <Button onClick={() => handleClassifica('Champions', 'A')} startIcon={<EmojiEvents />} sx={isXs ? { fontSize: '10px' } : {}}>{isXs ? 'Girone A' : 'Champions Girone A'}</Button>
                    <Button onClick={() => handleClassifica('Champions', 'B')} startIcon={<EmojiEvents />} sx={isXs ? { fontSize: '10px' } : {}}>{isXs ? 'Girone B' : 'Champions Girone B'}</Button>
                  </ButtonGroup>
                </Box>
              </Grid>
            </Zoom >
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', '& > *': { m: 1, }, }}>
                  <ButtonGroup size="small" color='primary' aria-label="Small button group">
                    ...
                    <Tooltip title="Calendario girone 2">
                      <Button onClick={() => handleCalendario(2, false, false)} startIcon={<LooksTwoOutlined />} >&nbsp;</Button>
                    </Tooltip>
                    <Tooltip title="Calendario girone 3">
                      <Button onClick={() => handleCalendario(3, false, false)} startIcon={<Looks3Outlined />} >&nbsp;</Button>
                    </Tooltip>
                    <Tooltip title="Calendario girone 4">
                      <Button onClick={() => handleCalendario(4, false, false)} startIcon={<Looks4Outlined />} >&nbsp;</Button>
                    </Tooltip>
        ...
        {frame === FrameType.albo && (
          <>
            ...
            <Slide direction={'down'} in={frame === FrameType.albo} >
              <Grid item xs={12} sm={12} sx={!isXs ? { pl: '2px', pr: '15px', pt: '15px' } : {}}>
                <Albo></Albo>
              </Grid>
            </Slide>
          </>
        ...
        {frame === FrameType.schieraFormazione && session?.user && (
          <Zoom in={frame === FrameType.schieraFormazione} >
            <Grid item xs={12} sm={12}>
              {isXs ? (
                <FormazioneXs onActionChange={handleChangeFrame} idSquadra={session?.user.idSquadra} squadra={session?.user?.squadra} />
              ) : (
                <Formazione onActionChange={handleChangeFrame} idSquadra={session?.user.idSquadra} squadra={session?.user?.squadra} />
              )}
            </Grid>
          </Zoom>
        )}
        {frame === FrameType.rosa && idSquadra && squadra && (
          <Zoom in={true}>
            <Grid item xs={12} sm={12}>
              <Rosa onActionGoToStatistica={handleChangeStatistica} onActionChange={handleChangeRosa} onActionGoToGiocatore={handleChangeGiocatori} idSquadra={idSquadra} squadra={squadra} />
            </Grid>
          </Zoom>
        )}
        ...
        {frame === FrameType.statisticheSquadra && idSquadra && (
          <Zoom in={frame === FrameType.statisticheSquadra} >
            <Grid item xs={12} sm={12}>
              <StatisticaSquadra onActionGoToFormazione={handleChangeFrame} onActionGoToRosa={handleChangeRosa} onActionChangePartita={handleChangePartita} idSquadra={idSquadra} />
            </Grid>
          </Zoom>
        )}
        {frame === FrameType.giocatori && (
          <Zoom in={frame === FrameType.giocatori} >
            <Grid item xs={12} sm={12}>
              <Giocatori onActionChange={handleChangeFrame} idGiocatore={idGiocatore} />
            </Grid>
          </Zoom>
        )}
      </Grid>

      <Modal title={titleModalCalendario} open={openModalCalendario} onClose={handleModalClose} width={isXs ? '98%' : '1266px'} height={isXs ? '98%' : ''} >
        <Divider />
