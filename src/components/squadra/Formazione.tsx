import { AutoGraph, Filter1, Filter2, Filter3, Filter4, Filter5, Filter6, Home, HourglassTop, Save } from "@mui/icons-material";
import { Box, CircularProgress, Grid, IconButton, List, ListItem, ListItemText, MenuItem, Select, type SelectChangeEvent, Tooltip, Typography, Stack, Button, Snackbar, Alert, useTheme, useMediaQuery, Divider } from "@mui/material";
...
function Formazione({ onActionChange: onActionActive, idSquadra, squadra }: FormazioneProps) {
    const theme = useTheme();
    const isXs = useMediaQuery(theme.breakpoints.down('md'));
    const [idGiocatoreStat, setIdGiocatoreStat] = useState<number>();
    const [openModalCalendario, setOpenModalCalendario] = useState(false);
    
    const calendarioProssima = api.formazione.getGiornateDaGiocare.useQuery(undefined, { refetchOnWindowFocus: false, refetchOnReconnect: false });
    ...
    const formazioneList = api.formazione.get.useQuery({ idTorneo: idTorneo! }, { enabled: !!idTorneo, refetchOnWindowFocus: false, refetchOnReconnect: false });
    const rosaList = api.squadre.getRosa.useQuery({ idSquadra: idSquadra, includeVenduti: false }, { enabled: enableRosa, refetchOnWindowFocus: false, refetchOnReconnect: false });
    ...
    const [draggedItem, setDraggedItem] = useState<GiocatoreFormazioneType | null>(null);
    ...
    useEffect(() => {
        if (calendarioProssima.data) {
            if (calendarioProssima.data.length > 0 && checkDataFormazione(calendarioProssima.data[0]?.data)) {
                setEnableRosa(true);
    ...
    useEffect(() => {
        if (formazioneList.data) {
            ...
            setRosa(sortPlayersByRoleDescThenCostoDesc(formazioneList.data.giocatori.filter(c => !c.titolare && c.riserva === null)));
            setPanca(sortPlayersByRoleDescThenRiserva(formazioneList.data.giocatori.filter(c => !c.titolare && c.riserva)));
        }
    ...
    const handleClickPlayer = async (playerClicked: GiocatoreFormazioneType) => {
        ...
        if (rosa.findIndex(c => c.idGiocatore === playerClicked.idGiocatore) > -1 && checkModulo(playerClicked.ruolo)) { //va da rosa a campo
            playerClicked.titolare = true;
            await updateLists(playerClicked, campo, setCampo, rosa, setRosa, false);
        }
        if (rosa.findIndex(c => c.idGiocatore === playerClicked.idGiocatore) > -1 && !checkModulo(playerClicked.ruolo)) { //va da rosa a panca
            playerClicked.riserva = 100;
            await updateLists(playerClicked, panca, setPanca, rosa, setRosa, true);
        }
        if (campo.findIndex(c => c.idGiocatore === playerClicked.idGiocatore) > -1) { //va da campo a rosa
            await updateLists(playerClicked, rosa, setRosa, campo, setCampo, true);
        }
        if (panca.findIndex(c => c.idGiocatore === playerClicked.idGiocatore) > -1) { //va da panca a rosa
    ...
    const handleDragStart = async (player: GiocatoreFormazioneType) => {
        ...
        if (rosa.findIndex(c => c.idGiocatore === player.idGiocatore) > -1 && checkModulo(player.ruolo)) { //va da rosa a campo
            setStyleCampo((prevStyle) => ({ ...prevStyle, borderWidth: '3px', borderStyle: 'dashed' }));
        }
        if (rosa.findIndex(c => c.idGiocatore === player.idGiocatore) > -1 && !checkModulo(player.ruolo)) { //va da rosa a panca
            setStylePanca((prevStyle) => ({ ...prevStyle, borderWidth: '3px', borderStyle: 'dashed' }));
        }
        if (campo.findIndex(c => c.idGiocatore === player.idGiocatore) > -1) { //va da campo a rosa
            setStyleRosa((prevStyle) => ({ ...prevStyle, borderWidth: '3px', borderStyle: 'dashed' }));
        }
        if (panca.findIndex(c => c.idGiocatore === player.idGiocatore) > -1) { //va da panca a rosa
            setStyleRosa((prevStyle) => ({ ...prevStyle, borderWidth: '3px', borderStyle: 'dashed' }));
        }
    ...
    const handleDrop = async () => {
        ...
        if (draggedItem) {
            ...
            if (rosa.findIndex(c => c.idGiocatore === draggedItem.idGiocatore) > -1 && checkModulo(draggedItem.ruolo)) { //va da rosa a campo
                draggedItem.titolare = true;
                await updateLists(draggedItem, campo, setCampo, rosa, setRosa, false);
            }
            if (rosa.findIndex(c => c.idGiocatore === draggedItem.idGiocatore) > -1 && !checkModulo(draggedItem.ruolo)) { //va da rosa a panca
                draggedItem.riserva = 100;
                await updateLists(draggedItem, panca, setPanca, rosa, setRosa, true);
            }
            if (campo.findIndex(c => c.idGiocatore === draggedItem.idGiocatore) > -1) { //va da campo a rosa
                await updateLists(draggedItem, rosa, setRosa, campo, setCampo, true);
            }
            if (panca.findIndex(c => c.idGiocatore === draggedItem.idGiocatore) > -1) { //va da panca a rosa
                await updateLists(draggedItem, rosa, setRosa, panca, setPanca, false, true);
            }
        }
    };

    const sortPlayersByRoleDescThenCostoDesc = (players: GiocatoreFormazioneType[]) => {
    ...
    const sortPlayersByRoleDescThenRiserva = (players: GiocatoreFormazioneType[]) => {
    ...
    const updateLists = async (playerSelected: GiocatoreFormazioneType,
        ...
        if (playerSelected && !targetArray.find(c => c.idGiocatore === playerSelected.idGiocatore)) {
            const updatedSourceArray = sourceArray.filter(player => player.idGiocatore !== playerSelected.idGiocatore);
            setSourceArray(updatedSourceArray);
            const updatedTargetArray = [...targetArray, playerSelected];
            orderSourceList ? sortPlayersByRoleDescThenRiserva(updatedSourceArray) : setSourceArray(updatedSourceArray);
            orderTargetList ? setTargetArray(sortPlayersByRoleDescThenRiserva(updatedTargetArray)) : setTargetArray(updatedTargetArray);
        }
    }
                                    ...
                                    <Draggable key={player.idGiocatore} nodeRef={nodeRef}>
                                        ...
                                        >
                                            <ListItem sx={{ cursor: 'pointer', zIndex: 2, paddingTop: '0px', paddingBottom: '0px' }}>
                                                <Image src={player.urlCampioncinoSmall} width={42} height={42} alt={player.nome} />
                                                <ListItemText primary={player.nome} secondary={`${player.nomeSquadraSerieA}`}></ListItemText>
                                            </ListItem>
                                        </div>
                                    </Draggable>
                                </Grid>
                        ...
                        {filteredPanca.map((player) => (
                            <Draggable key={player.idGiocatore} nodeRef={nodeRef}>
                                ...
                                >
                                    <ListItem sx={{ cursor: 'pointer', zIndex: 2, paddingTop: '0px', paddingBottom: '0px' }}>
                                        <Image src={player.urlCampioncinoSmall} width={42} height={42} alt={player.nome} />
                                        <ListItemText primary={player.nome} secondary={`${player.ruoloEsteso} (${player.nomeSquadraSerieA?.toUpperCase().substring(0, 3)})`}></ListItemText>
                                        {player.riserva === 1 ? <Filter1 color="success"></Filter1> :
                                            player.riserva === 2 ? <Filter2 color="warning"></Filter2> :
                                                player.riserva === 3 ? <Filter3 color="error"></Filter3> :
                                                    player.riserva === 4 ? <Filter4 color="secondary"></Filter4> :
                                                        player.riserva === 5 ? <Filter5 color="info"></Filter5> :
                                                            player.riserva === 6 ? <Filter6 color="primary"></Filter6> :
                                                                <></>
                                        }
                                    </ListItem>
                                </div>
                    ...
                    return (
                        <Draggable key={player.idGiocatore} nodeRef={nodeRef}>
                            <div
                                ...
                                draggable
                                ref={nodeRef}
                                style={{ cursor: 'pointer', zIndex: 2, minWidth: '120px', position: 'absolute', ...style }}
                            >
                                <Stack direction={'column'} justifyContent="space-between" alignItems="center">
                                    <Image src={player.urlCampioncinoSmall} key={player.idGiocatore} width={48} height={48} alt={player.nome} />
                                    <Typography variant="body2" sx={{ color: 'white', backgroundColor: '#2e865f', opacity: 0.8, padding: '2px' }}>{player.nome}</Typography>
                                </Stack>
                            </div>
                        </Draggable>
                    )
    ...
    function checkModulo(ruolo: string) {
        ...
        const maxRuolo = ruolo === 'P' ? 1 : ruolo === 'D' ? parseInt(moduloSplitted[0] ?? '3') : ruolo === 'C' ? parseInt(moduloSplitted[1] ?? '4') : parseInt(moduloSplitted[2] ?? '3');
    ...
    function correctFormazione(modulo: Moduli) {
        ...
        moduloSplitted.map((c, index) => {
            ...
            const giocatori = campo.filter(g => g.ruolo === ruoliList[index + 1]).slice(numRuolo);
            giocatori.forEach(playerToRemove => {
                rosa.push(playerToRemove);
                const playerIndex = campo.findIndex(g => g.idGiocatore === playerToRemove.idGiocatore);
                if (playerIndex !== -1) {
    ...
    return (
                ...
                <>
                    <Grid item xs={6}>
                        <Stack direction={'row'}
                            ...
                            alignItems={'center'}
                            sx={{ pb: '5px' }}>
                            <Typography variant={'h3'}>Formazione {squadra} {giornate.length === 1 && ` - ${giornate[0]?.Title}`}</Typography>
                            {giornate.length > 1 && (
                                <Select size='small'
                                    variant='outlined'
                                ...
                                >
                                    {giornate.map((g, index) => (
                                        <MenuItem value={g.idTorneo} key={`giornata_${g.idTorneo}`} selected={index === 0}>{`${g.Title}`}</MenuItem>
                                    ))}
                                </Select>
                            )}
                            <Box component="form" onSubmit={handleSave} noValidate>
                                <Button type="submit" disabled={saving} endIcon={!saving ? <Save /> : <HourglassTop />}
                                    variant="contained" color='error' size="medium" sx={{ ml: '5px' }}>{saving ? 'Attendere...' : 'Salva'}</Button>
                            </Box>
                        </Stack>
                    </Grid>
                    ...
                    {idSquadra === session?.user?.idSquadra ? (
                        <>
                            ...
                            <Grid item sm={4} xs={12} sx={{ display:'flex', justifyContent:"flex-end"}}>
                                <Box sx={styleCampo}
                                    onDragOver={(e) => e.preventDefault()}
                                    onDrop={handleDrop}>
                                    <Grid item xs={12}>
                                        <Select size='small'
                                            ...
                                            margin='dense'
                                            required
                                            sx={{ m: '2px', backgroundColor: '#2e865f', color: 'white', opacity: 0.8 }}
                                            name="modulo"
                                            onChange={handleSetModulo}
                                            value={modulo}
                                        >
                                            {moduliList.map(moduloOption => (
                                                <MenuItem value={moduloOption} key={`modulo_${moduloOption}`}>{`Modulo ${moduloOption}`}</MenuItem>
                                            ))}
                                        </Select>
                                    </Grid>
                                    {renderCampo(['P'])}
                        ...
                        <Grid item xs={12} display={'flex'} justifyContent={'center'} sx={{ mt: '30px' }}>
                            <Typography variant="h3">Accesso non consentito</Typography>
                        </Grid>
                    )}
                </>
            ) : (
                <Grid item xs={12} display={'flex'} justifyContent={'center'} sx={{ mt: '30px' }}>
                    <Typography variant="h3" color="error">{message}</Typography>
                </Grid>
            )}
        </Grid>
        ...
        <Modal title={'Statistica giocatore'} open={openModalCalendario} onClose={handleModalClose} width={isXs ? '98%' : '1266px'} height={isXs ? '98%' : ''} >
                ...
                <Box sx={{ mt: 1, gap: '0px', flexWrap: 'wrap' }}>
                    <Giocatori idGiocatore={idGiocatoreStat} onActionChange={() => { if (false) {} }} removeNav={true}></Giocatori>
                </Box>
            </Modal>
        </>
    )
