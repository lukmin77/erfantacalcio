...
import { Box, Card, CardContent, CardMedia, CircularProgress, Grid, IconButton, Tab, Tabs, Tooltip, Typography, useMediaQuery } from "@mui/material";
...
function Rosa({ onActionChange: onActionActive, onActionGoToStatistica: onActionStatistica, onActionGoToGiocatore: onActionGiocatore, idSquadra, squadra }: RosaProps) {
    const theme = useTheme();
    const isXs = useMediaQuery(theme.breakpoints.down('md'));
    const rosaList = api.squadre.getRosa.useQuery({ idSquadra: idSquadra, includeVenduti: true }, { refetchOnWindowFocus: false, refetchOnReconnect: false });
    ...
    const renderRosa = (roles: string[], isVenduto: boolean) => {
        const filteredRosa = rosa.filter(player => roles.includes(player.ruolo) && player.isVenduto === isVenduto);
        return (
                    ...
                    <Grid key={index} item xs={6} sm={1.5} sx={{ display: 'flex', justifyContent: 'center', p:'10px', mb: '10px', flexWrap: 'wrap' }}>
                        <Card sx={{ marginBottom: '1px' }}>
                            ...
                            <CardContent sx={{ paddingBottom: '0px' }}>
                                <Typography gutterBottom variant="body2" sx={{ fontSize: '10px', display: 'flex', justifyContent: 'center' }} component="div">
                                    {giocatore.nome}
                                </Typography>
                                <Typography gutterBottom variant="body1" sx={{ fontSize: '10px', display: 'flex', justifyContent: 'center' }} component="div">
                ...
                <>
                    ...
                    <Grid item xs={6} sx={{ display: 'flex', justifyContent: 'flex-end', }}>
                        ...
                        <Tooltip title="Statistiche">
                            <IconButton onClick={() => handleActionStatistica(FrameType.statisticheSquadra, idSquadra)}>
