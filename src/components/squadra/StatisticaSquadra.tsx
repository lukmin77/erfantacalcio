...
import { Box, Card, CardContent, CardHeader, CardMedia, CircularProgress, Divider, Grid, IconButton, Stack, Tab, Tabs, Tooltip, Typography, useTheme } from "@mui/material";
...
interface StatisticaSquadraProps {
    ...
    onActionGoToRosa: (action: FrameType, idSquadra: number, squadra: string) => void;
...
function StatisticaSquadra({ onActionChangePartita: onActionActivePartita, onActionGoToRosa: onActionActiveRosa, onActionGoToFormazione: onActionActiveFormazione, idSquadra }: StatisticaSquadraProps) {
    const { data: session } = useSession();
    const theme = useTheme();
    const apiSquadra = api.squadre.get.useQuery({ idSquadra: idSquadra }, { refetchOnWindowFocus: false, refetchOnReconnect: false });
    const apiAlbo = api.albo.get.useQuery({ idSquadra: idSquadra }, { refetchOnWindowFocus: false, refetchOnReconnect: false });
    const apiPartite = api.calendario.listPartiteBySquadra.useQuery({ idSquadra: idSquadra }, { refetchOnWindowFocus: false, refetchOnReconnect: false });
    ...
    const handleActionRosa = (newFrame: FrameType, idSquadra: number, squadra: string) => {
    ...
    function getStatsPartite(torneo: string, datiPartite: GiornataType[]) {
        ...
        const pareggi = datiPartite.filter(c => c.Torneo === torneo).flatMap(c => c.partite.filter(c => c.idHome === idSquadra && (c.golHome ?? 0) === (c.golAway ?? 0))).length + datiPartite.flatMap(c => c.partite.filter(c => c.idAway === idSquadra && (c.golAway ?? 0) === (c.golHome ?? 0))).length;
        const perse = datiPartite.filter(c => c.Torneo === torneo).flatMap(c => c.partite.filter(c => c.idHome === idSquadra && (c.golHome ?? 0) < (c.golAway ?? 0))).length + datiPartite.flatMap(c => c.partite.filter(c => c.idAway === idSquadra && (c.golAway ?? 0) < (c.golHome ?? 0))).length;
            ...
            {datiSquadra && datiAlbo && datiPartite && (
                <>
                    ...
                    <Grid item xs={3} sm={3} sx={{ display: 'flex', justifyContent: 'flex-end', }}>
                        ...
                        <Tooltip title="Rosa">
                            <IconButton onClick={() => handleActionRosa(FrameType.rosa, idSquadra, datiSquadra.squadra)}>
                                <Diversity1 color='success' />
                            </IconButton>
                        </Tooltip>
                        <Tooltip title="Home" placement="top-start">
                            <IconButton onClick={() => handleActionPartita(FrameType.defaultHome, 0)}>
                                <Home color='primary' />
                            </IconButton>
                        </Tooltip>
                    </Grid>
                    <Grid item xs={12} sm={3}>
                        <Card>
                            <CardHeader
                                title={datiSquadra.presidente} titleTypographyProps={{ variant: 'h4' }}
                    ...
                    <Grid item xs={12} sm={9}>
                        <Grid container spacing={0} sx={{ m: '15px' }}>
                            ...
                            <Grid item sm={6} xs={12}>
                                <Box flexGrow={1}>
                                    <Typography variant="body2" display={'flex'} justifyContent={'center'}>Risultati Campionato</Typography>
                                    <PieChart
