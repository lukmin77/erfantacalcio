...
import { iRosaStats, type iGiocatoreStats } from '~/types/giocatori'
...
export const giocatoriRouter = createTRPCRouter({
  ...
  listStatisticheSquadra: publicProcedure
    .input(
      z.object({
        id_squadra: z.number(),
      }),
      ...
      try {
        ...
        return stat
          ? stat.map((player) => ({
              ...
              id: player.idgiocatore,
              maglia: `/images/maglie/${player.maglia}`,
              gol: player.ruolo === 'P' ? -Number(player.golsubiti) : Number(player.golfatti),
            }))
          : []
      } catch (error) {
        Logger.error('Si è verificato un errore', error)
