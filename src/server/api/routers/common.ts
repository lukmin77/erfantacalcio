...
import { type GiornataType, type iCalendarioPartite, type iPartita } from '~/types/common';
...
export async function chiudiTrasferimentoGiocatore(idGiocatore: number, chiusuraStagione: boolean) {
    try {
        ...
        const oldTrasferimento = await prisma.trasferimenti.findFirst({
            select: {
                dataAcquisto: true, idTrasferimento: true,
        ...
        if (oldTrasferimento) {
            Logger.debug('dati ultimo trasferimento idgiocatore: ' + idGiocatore, oldTrasferimento);
            //cerca i voti nel periodo dall'ultima data di acuisto ad oggi
            const voti = await prisma.voti.findMany({
                select: {
                    voto: true, gol: true, assist: true, //idVoto: true,
                    Calendario: {
                        select: {
            ...
            if (voti.length > 0) {
                Logger.debug('voti ultimo trasferimento idgiocatore: ' + idGiocatore, voti);
                //sui voti precedentemente trovati calcolo media, gol, assist e giocate
                const oldStatistica = voti.reduce((acc, curr) => {
                ...
                oldStatistica.golTotali = oldTrasferimento.Giocatori.ruolo === 'P' ? oldStatistica.golTotali : oldStatistica.golTotali / 3;

                Logger.debug('statistica ultimo trasferimento idgiocatore: ' + idGiocatore, oldStatistica);
...
export async function getProssimaGiornataSerieA(isGiocata: boolean, orderType: 'asc' | 'desc') {
...
export async function getRosaDisponibile(idSquadra: number) {
    const query = await prisma.trasferimenti.findMany({
        ...
        orderBy: [{ Giocatori: { ruolo: 'desc' } }, { costo: 'desc' }, { Giocatori: { nome: 'asc' } }]
    });
    ...
    return query.map<GiocatoreType>(giocatore => ({
        ...
        isVenduto: false,
        urlCampioncino: normalizeCampioncinoUrl(Configurazione.urlCampioncino, giocatore.Giocatori.nome, giocatore.Giocatori.nomeFantaGazzetta),
        urlCampioncinoSmall: normalizeCampioncinoUrl(Configurazione.urlCampioncinoSmall, giocatore.Giocatori.nome, giocatore.Giocatori.nomeFantaGazzetta),
...
export async function mapCalendario(result: iCalendarioPartite[]): Promise<GiornataType[]> {
    return result.map(c => ({
        ...
        Descrizione: getDescrizioneTorneo(c.Tornei.nome, c.giornata, c.giornataSerieA, c.Tornei.gruppoFase),
...
export function getDescrizioneTorneo(nome: string, giornata: number, giornataSerieA: number, gruppoFase?: string | null): string {
...
export function getTorneoTitle(nome: string, giornata: number, gruppoFase?: string | null): string {
    if (gruppoFase === null || gruppoFase === undefined) {
...
export function getGiocatoriVotoInfluente(giocatoriFormazione: {
    ruolo: string;
    idVoto: number;
    voto: Decimal | null;
    ammonizione: Decimal;
...
export function getBonusSenzaVoto(giocatoriInfluenti: number) {
    return (11 - giocatoriInfluenti > Configurazione.maxSostituzioni ? Configurazione.maxSostituzioni : 11 - giocatoriInfluenti) * Configurazione.bonusSenzaVoto;
}
...
export function getGolSegnati(fantapunti: number): number {
    ...
    if (fantapunti >= soglieGol.soglia1 && fantapunti < soglieGol.soglia2) {
        gol = 1;
    } else if (fantapunti >= soglieGol.soglia2 && fantapunti < soglieGol.soglia3) {
        gol = 2;
    } else if (fantapunti >= soglieGol.soglia3 && fantapunti < soglieGol.soglia4) {
        gol = 3;
    } else if (fantapunti >= soglieGol.soglia4 && fantapunti < soglieGol.soglia5) {
        gol = 4;
    } else if (fantapunti >= soglieGol.soglia5 && fantapunti < soglieGol.soglia6) {
        gol = 5;
    } else if (fantapunti >= soglieGol.soglia6 && fantapunti < soglieGol.soglia7) {
        gol = 6;
    } else if (fantapunti >= soglieGol.soglia7 && fantapunti < soglieGol.soglia8) {
        gol = 7;
...
export async function getTabellino(idFormazione: number) {
    const giocatoriFormazione = (await prisma.voti.findMany({
        include: {
            Giocatori: {
                select: {
                    ruolo: true
        ...
        isSostituito: false,
        isVotoInfluente: v.titolare && v.voto && v.voto.toNumber() > 0 ? true : false
    }));

    const countRiserve = getCountRiserve(getGiocatoriVotoInfluente(giocatoriFormazione).length);

    if (getGiocatoriVotoInfluente(giocatoriFormazione).length < 11) {
        let iRiserve = 0;
        const titolariSenzaVoto = giocatoriFormazione.filter(c => c.titolare && c.voto && c.voto.toNumber() === 0);
        ...
        for (const riserva of riserveConVoto) {
            const giocatoreRuolo = titolariSenzaVoto.find(c => c.ruolo === riserva.ruolo);
            if (giocatoreRuolo) {
                titolariSenzaVoto.splice(titolariSenzaVoto.findIndex(r => r.idVoto === giocatoreRuolo.idVoto), 1);
                if (riserveConVoto.find(c => c.ruolo === giocatoreRuolo.ruolo && !c.isVotoInfluente)) {
...
export function getCountRiserve(titolariInfluenti: number) {
    return 11 - titolariInfluenti > Configurazione.maxSostituzioni ? Configurazione.maxSostituzioni : 11 - titolariInfluenti;
}
