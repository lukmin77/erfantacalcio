...
import { getProssimaGiornata, getProssimaGiornataSerieA, getRosaDisponibile } from "./common";
...
export const formazioneRouter = createTRPCRouter({
  ...
  getGiornateDaGiocare: protectedProcedure
    .query(async (opts) => {
      try {
        ...
        const giornateFiltrate = prossimeGiornate.filter(giornata =>
          giornata.partite.some(partita =>
            partita.idHome === idSquadraUtente || partita.idAway === idSquadraUtente
          )
        );
  ...
  get: protectedProcedure
    ...
    .query(async (opts) => {
      ...
      try {
        const giornataSerieA = await getProssimaGiornataSerieA(false, 'asc');
        const prossimoCalendario = (await getProssimaGiornata(giornataSerieA)).find(c => c.idTorneo === idTorneo);
        const prossimaPartita = prossimoCalendario?.partite.find(c => c.idHome === idSquadraUtente || c.idAway === idSquadraUtente);
        if (!prossimaPartita || !prossimoCalendario) {
          ...
          const giocatoriSchierati = await prisma.voti.findMany({
            select: {
              idGiocatore: true, titolare: true, riserva: true
            },
            where: {
              AND: [
          ...
          const formazione: GiocatoreFormazioneType[] = rosa.map(r => ({
            ...
            titolare: giocatoriSchierati.find(g => g.idGiocatore === r.idGiocatore)?.titolare ?? false,
            riserva: giocatoriSchierati.find(g => g.idGiocatore === r.idGiocatore)?.riserva ?? null,
          }));
  ...
  create: protectedProcedure
    ...
    .mutation(async (opts) => {
      ...
      try {
        const idFormazione = (await prisma.formazioni.findFirst({
        ...
        Logger.info(`Eliminazione formazione per idSquadra: ${idSquadra} e idPartita:${idPartita}`);
        ...
        Logger.info(`recupero idCalendario per idPartita: ${idSquadra}:${calendario?.idCalendario}`);
        ...
        if (calendario) {
          await Promise.all(giocatori.map(async (c) => {
          ...
          Logger.info(`Inseriti giocatori in tabella voti con idFormazione: ${formazione.idFormazione}`);
          ...
          if (env.MAIL_ENABLED) {
            ...
            const avversario = idSquadra === calendario.Utenti_Partite_idSquadraHToUtenti?.idUtente ? calendario.Utenti_Partite_idSquadraHToUtenti?.presidente : calendario.Utenti_Partite_idSquadraAToUtenti?.presidente;
            const to = idSquadra === calendario.Utenti_Partite_idSquadraHToUtenti?.idUtente ? calendario.Utenti_Partite_idSquadraAToUtenti?.mail : calendario.Utenti_Partite_idSquadraHToUtenti?.mail;
            ...
            if (to)
              await SendMail(to, subject, htmlMessage);
            else{
          ...
          Logger.warn(`Calendario non trovato, impossibile procedere con l'inserimento della formazione`);
      }
      catch (error) {
        Logger.error('Si è verificato un errore', error);
        throw error;
