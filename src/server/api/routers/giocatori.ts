...
import { z } from 'zod';
import { getRuoloEsteso, normalizeCampioncinoUrl, normalizeNomeGiocatore } from "~/utils/helper";
import { deleteGiocatore, deleteVotiGiocatore, getGiocatoreById } from "./common";
      ...
      try {
        const giocatore = await prisma.giocatori.upsert({
          ...
          update: {
            ...
            nomeFantaGazzetta: opts.input.nomeFantagazzetta ? normalizeNomeGiocatore(opts.input.nomeFantagazzetta) : null,
    ...
    .query(async (opts) => {
      try {
        ...
        switch (opts.input.ruolo) {
          case 'P':
            maxGiocate = (await prisma.statsA.aggregate({ _max: { giocate: true } }))._max?.giocate ?? 0;
            sogliaGiocate = maxGiocate * (Configurazione.percentualeMinimaGiocate / 100);
            stat = await prisma.statsP.findMany({
              select: {
                media: true, mediabonus: true, golfatti: true, golsubiti: true, assist: true,
                ammonizioni: true, espulsioni: true, giocate: true, ruolo: true, nome: true, nomefantagazzetta: true,
                idgiocatore: true, maglia: true, squadraSerieA: true, squadra: true, idSquadra: true
              },
              where: {
                giocate: {
              ...
              orderBy: [
                { 'media': 'desc' }, { 'mediabonus': 'desc' }, { 'giocate': 'desc' }
              ],
              skip: 0,
      ...
      try {
        ...
        if (giocatore) {
          const result = await prisma.voti.aggregate({
            _avg: { voto: true },
            _sum: { ammonizione: true, espulsione: true, gol: true, assist: true },
          ...
          return {
            ...
            gol: giocatore.ruolo === 'P' ? Number(result._sum.gol) * Configurazione.bonusGolSubito : Number(result._sum.gol) / Configurazione.bonusGol,
            assist: Number(result._sum.assist) / Configurazione.bonusAssist,
            ammonizioni: Math.abs(Number(result._sum.ammonizione) / Configurazione.bonusAmmonizione),
            espulsioni: Math.abs(Number(result._sum.espulsione) / Configurazione.bonusEspulsione),
            ...
            isVenduto: false,
            urlCampioncino: normalizeCampioncinoUrl(Configurazione.urlCampioncino, giocatore.nome, giocatore.nomeFantaGazzetta),
            urlCampioncinoSmall: normalizeCampioncinoUrl(Configurazione.urlCampioncinoSmall, giocatore.nome, giocatore.nomeFantaGazzetta),
          };
        }
