...
import { getTorneo, getDescrizioneTorneo, getTorneoTitle, getTorneoSubTitle, getBonusModulo, getGiocatoriVotoInfluente, getBonusSenzaVoto, getGolSegnati, getTabellino } from "./common";
        ...
        if (partita?.idSquadraH && partita?.idSquadraA) {
          ...
          await prisma.$transaction([
            ...
            prisma.partite.update({
              data: {
                puntiH: getPunti(infoCalendario?.Calendario.Tornei.hasClassifica ?? false, opts.input.multaHome, opts.input.golHome, opts.input.golAway),
                puntiA: getPunti(infoCalendario?.Calendario.Tornei.hasClassifica ?? false, opts.input.multaAway, opts.input.golAway, opts.input.golHome),
          ...
          Logger.info(`Aggiornate formazioni, calendario e partite per idpartita: ${opts.input.idPartita}`);

          if (infoCalendario?.Calendario.Tornei.hasClassifica) {
            await UpdateClassifica(idSquadraHome, infoCalendario.Calendario.Tornei.idTorneo);
            Logger.info(`Aggiornate classifica e utenti (multe) per idsquadraHome: ${idSquadraHome} e idTorneo: ${infoCalendario.Calendario.Tornei.idTorneo}`);
            await UpdateClassifica(idSquadraAway, infoCalendario.Calendario.Tornei.idTorneo);
            Logger.info(`Aggiornate classifica e utenti (multe) per idsquadraAway: ${idSquadraAway} e idTorneo: ${infoCalendario.Calendario.Tornei.idTorneo}`);
          }
        }
    ...
    .query(async (opts) => {
      try {
        const result = await prisma.calendario.findUnique({
          select: {
            idCalendario: true, giornata: true, giornataSerieA: true, ordine: true, data: true, dataFine: true, hasSovrapposta: true, girone: true, hasGiocata: true,
            Tornei: {
        ...
        if (result)
          return {
            ...
            partite: await mapPartite(result.Partite, opts.input.includeTabellini, opts.ctx.session?.user?.ruolo === RuoloUtente.contributor ? false : opts.input.backOfficeMode),
            Torneo: getTorneo(result.Tornei.nome, result.Tornei.gruppoFase),
            Descrizione: getDescrizioneTorneo(result.Tornei.nome, result.giornata, result.giornataSerieA, result.Tornei.gruppoFase),
            Title: getTorneoTitle(result.Tornei.nome, result.giornata, result.Tornei.gruppoFase),
    ...
    .query(async (opts) => {
      try {
        if (opts.input.idSquadra) {
          const resultFormazione = await getFormazione(opts.input.idPartita, opts.input.idSquadra);
          if (resultFormazione) {
            const giocatoriInfluenti = await getTabellino(resultFormazione.idFormazione);

            if (giocatoriInfluenti) {
              const fantapunti = getGiocatoriVotoInfluente(giocatoriInfluenti).reduce((acc, cur) => acc + (cur.votoBonus ?? 0), 0);
              return {
                ...
                giocatoriInfluenti: getGiocatoriVotoInfluente(giocatoriInfluenti).length,
                bonusSenzaVoto: getBonusSenzaVoto(getGiocatoriVotoInfluente(giocatoriInfluenti).length),
...
function mapPartite(partite: iPartita[], includeTabellini: boolean, backOfficeMode: boolean) {
  return Promise.all(partite.map(async (p) => {
    ...
    const tabellinoHome = includeTabellini && formazioneHome?.idFormazione ? await getTabellino(formazioneHome?.idFormazione) : [];
    const tabellinoAway = includeTabellini && formazioneAway?.idFormazione ? await getTabellino(formazioneAway?.idFormazione) : [];
    const fantapuntiHome = includeTabellini ? getGiocatoriVotoInfluente(tabellinoHome).reduce((acc, cur) => acc + (cur.votoBonus ?? 0), 0) : 0;
    const fantapuntiAway = includeTabellini ? getGiocatoriVotoInfluente(tabellinoAway).reduce((acc, cur) => acc + (cur.votoBonus ?? 0), 0) : 0;
    const bonusModuloHome = includeTabellini && fantapuntiHome > 0 ? getBonusModulo(formazioneHome?.modulo ?? '') : 0;
    const bonusModuloAway = includeTabellini && fantapuntiAway > 0 ? getBonusModulo(formazioneAway?.modulo ?? '') : 0;
    const bonusSenzaVotoHome = includeTabellini && fantapuntiHome > 0 ? getBonusSenzaVoto(getGiocatoriVotoInfluente(tabellinoHome).length) : 0;
    const bonusSenzaVotoAway = includeTabellini && fantapuntiAway > 0 ? getBonusSenzaVoto(getGiocatoriVotoInfluente(tabellinoAway).length) : 0;
    const totaleFantapuntiHome = includeTabellini && fantapuntiHome > 0 ? fantapuntiHome + bonusModuloHome + bonusSenzaVotoHome + (p.fattoreCasalingo && fantapuntiHome > 0 ? Configurazione.bonusFattoreCasalingo : 0) : 0;
    const totaleFantapuntiAway = includeTabellini && fantapuntiAway > 0 ? fantapuntiAway + bonusModuloAway + bonusSenzaVotoAway : 0;
    const golSegnatiHome = includeTabellini && backOfficeMode && fantapuntiHome > 0 ? getGolSegnati(totaleFantapuntiHome) : 0;
    const golSegnatiAway = includeTabellini && backOfficeMode && fantapuntiAway > 0 ? getGolSegnati(totaleFantapuntiAway) : 0;
...
async function UpdateClassifica(idSquadra: number, idTorneo: number) {
  const puntiH = (await prisma.partite.aggregate({
    _sum: { puntiH: true },
    where: {
      AND: [
  ...
  const puntiA = (await prisma.partite.aggregate({
    _sum: { puntiA: true },
    where: {
      AND: [
        { Calendario: { idTorneo: idTorneo } },
  ...
  const golFattiH = (await prisma.partite.aggregate({
    _sum: { golH: true },
    where: {
      AND: [
        { Calendario: { idTorneo: idTorneo } },
  ...
  const golSubitiH = (await prisma.partite.aggregate({
    _sum: { golA: true },
    where: {
      AND: [
        { Calendario: { idTorneo: idTorneo } },
  ...
  const golFattiA = (await prisma.partite.aggregate({
    _sum: { golA: true },
    where: {
      AND: [
        { Calendario: { idTorneo: idTorneo } },
  ...
  const golSubitiA = (await prisma.partite.aggregate({
    _sum: { golH: true },
    where: {
      AND: [
        { Calendario: { idTorneo: idTorneo } },
...
function getPunti(hasClassifica: boolean, multa: boolean, gol1: number, gol2: number): number {
  return hasClassifica ? multa ? 0 : gol1 > gol2 ? 3 : gol1 === gol2 ? 1 : 0 : 0;
}