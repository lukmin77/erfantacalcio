...
export const squadreRouter = createTRPCRouter({
    ...
    .query(async (opts) => {
      try {
        ...
        if (idSquadraUtenteConnesso) {
          const userSquadraIndex = utenti.findIndex(squadra => squadra.idUtente === idSquadraUtenteConnesso);
          if (userSquadraIndex !== -1) {
            const userSquadra = utenti.splice(userSquadraIndex, 1)[0];
            if (userSquadra)
              utenti = [userSquadra, ...utenti];
          }
        }
  ...
  getRosa: publicProcedure
    ...
    .query(async (opts) => {
      const { idSquadra, include } = { idSquadra: opts.input.idSquadra, include: opts.input.includeVenduti };
      try {
